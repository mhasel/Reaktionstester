...
void Clock_100kHz_Start(void) 
{
    ...
    Clock_100kHz_CLKEN |= Clock_100kHz_CLKEN_MASK;
	Clock_100kHz_CLKSTBY |= Clock_100kHz_CLKSTBY_MASK;
}
...
void Clock_100kHz_Stop(void) 
{
    ...
    Clock_100kHz_CLKEN &= (uint8)(~Clock_100kHz_CLKEN_MASK);
	Clock_100kHz_CLKSTBY &= (uint8)(~Clock_100kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_100kHz_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_StopBlock(void) 
{
    if ((Clock_100kHz_CLKEN & Clock_100kHz_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_100kHz__CFG3)
        CLK_DIST_AMASK = Clock_100kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_100kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_100kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_100kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_100kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_100kHz_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_100kHz_CLKSTBY &= (uint8)(~Clock_100kHz_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_100kHz_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_100kHz_GetSourceRegister();
    uint16 oldDivider = Clock_100kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_100kHz_CLKEN & Clock_100kHz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_100kHz_DIV_PTR, clkDivider);
                Clock_100kHz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_100kHz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_100kHz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_100kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock_100kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_100kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_SetModeRegister(uint8 modeBitMask) 
{
    Clock_100kHz_MOD_SRC |= modeBitMask & (uint8)Clock_100kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_100kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_100kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_100kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_100kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_100kHz_GetModeRegister(void) 
{
    return Clock_100kHz_MOD_SRC & (uint8)(Clock_100kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_100kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_100kHz_GetDividerRegister();
    uint8 oldSrc = Clock_100kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_100kHz_MOD_SRC |= CYCLK_SSS;
        Clock_100kHz_MOD_SRC =
            (Clock_100kHz_MOD_SRC & (uint8)(~Clock_100kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_100kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_100kHz_GetSourceRegister(void) 
{
    return Clock_100kHz_MOD_SRC & Clock_100kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_100kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_100kHz_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_100kHz_PHASE = clkPhase & Clock_100kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_100kHz_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_100kHz_GetPhaseRegister(void) 
{
    return Clock_100kHz_PHASE & Clock_100kHz_PHASE_MASK;
}
