...
/*******************************************************************************
* Function Name: Clock_10kHz_Start
********************************************************************************
*
* Summary:
...
void Clock_10kHz_Start(void) 
{
    ...
    Clock_10kHz_CLKEN |= Clock_10kHz_CLKEN_MASK;
	Clock_10kHz_CLKSTBY |= Clock_10kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_10kHz_Stop
********************************************************************************
*
* Summary:
...
void Clock_10kHz_Stop(void) 
{
    ...
    Clock_10kHz_CLKEN &= (uint8)(~Clock_10kHz_CLKEN_MASK);
	Clock_10kHz_CLKSTBY &= (uint8)(~Clock_10kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_10kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_10kHz_CLKEN & Clock_10kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_10kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_10kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_10kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_10kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_10kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_10kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_10kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_10kHz_GetSourceRegister();
    uint16 oldDivider = Clock_10kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_10kHz_CLKEN & Clock_10kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_10kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_10kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_10kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_10kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_10kHz_MOD_SRC |= modeBitMask & (uint8)Clock_10kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_10kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_10kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_10kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_10kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_10kHz_MOD_SRC & (uint8)(Clock_10kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_10kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_10kHz_GetDividerRegister();
    uint8 oldSrc = Clock_10kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_10kHz_MOD_SRC |= CYCLK_SSS;
        Clock_10kHz_MOD_SRC =
            (Clock_10kHz_MOD_SRC & (uint8)(~Clock_10kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_10kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_10kHz_MOD_SRC & Clock_10kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_10kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_10kHz_PHASE = clkPhase & Clock_10kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_10kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_10kHz_PHASE & Clock_10kHz_PHASE_MASK;
}
